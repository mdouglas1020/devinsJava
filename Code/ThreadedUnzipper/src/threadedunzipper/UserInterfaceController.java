...
public class UserInterfaceController implements Initializable {
    ...
    public void ready(Stage stage) {
        ...
        //this will handle when the window is closed and the thread is still running
    ...
    //NOTE: The Unzip button toggles between Unzip and End functions 
    
    @FXML
    private void handleUnzip(Event event) {
        if(unzip != null){
            //if unzipping then end it
            //the unzipping button will indicate End while unzipping
            unzip.end();
            return;
        }
        ...
        unzip.setOnNotification(new Notification() {
           ...
           public void handle(double percentUnzipped, UnzipState state, String message){
               progressBar.setProgress(percentUnzipped);
               statusTextArea.appendText(message + "\n");
               if(state != UnzipState.RUNNING){
                   //if not unzipping then set the unzip to null
                   unzip = null;
