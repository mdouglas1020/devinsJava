...
public class PlayerController implements Initializable {
    ...
    @FXML
    private void getNameReflection(ActionEvent event) {
        // http://docs.oracle.com/javase/8/docs/technotes/guides/reflection/enhancements.html
        
        if (currentVisualizer == null) return;
        
        try {
            
            reflectionText.setText("Has getName");
        } catch (NoSuchMethodException ex) {
            reflectionText.setText("No getName!");
        }
    }
    
    @FXML
    private void blahReflection(ActionEvent event) {
        if (currentVisualizer == null) return;
        
        try {
            
            reflectionText.setText("Has blah");
        } catch (NoSuchMethodException ex) {
            reflectionText.setText("No blah!");
        }
    }
    @FXML
    private void getAllMethods(ActionEvent event){
    ...
    @FXML
    private void getMethodAndDeclaration(ActionEvent event){

        try {
            Class[] arg = new Class[2]; 
            arg[0] = Integer.class; 
            arg[1] = AnchorPane.class; 
